...
class PythonHandler(AbstractHandler):
    def handle(self,file:str)->Optional[str]:
        try:
...
class CommandFactory:
    @staticmethod
    def create_command(language: str, file: str) -> Optional[Command]:
        if language == "Python":
...
class Invoker:
    ...
    def execute(self,file:str)->Optional[str]:
        language=self.handler.handle(file)
        if not language:
            return f"limbaj necunoascut"
        #creez comanda
        command=CommandFactory.create_command(language,file)
        if not command:
            return f"nu exista comanda pt acest limbaj"
...
if __name__=='__main__':
    ...
    invoker=Invoker(python_handler) 

    result=invoker.execute(input("file="))
    print(result)